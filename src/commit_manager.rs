const COMMIT_TEMPLATE: &'static str = "<req_type>(<opt_scope>)<opt_tagged>: <opt_body>\n <opt_footers>";
...
impl GitCommitManager {
    ...
    pub fn format_commit(&self, 
        ...
        r = r.replace("<opt_scope>",    match &commit_scope {Some(i) => i.as_str(), None => ""});
        r = r.replace("<opt_flagged>",  if is_flagged {"!"} else {""});
        r = r.replace("<opt_body>",     match &commit_body {Some(i) => i.as_str(), None => ""});
