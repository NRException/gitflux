...
impl GitTagManager {
    ...
    fn create_version_tag(&mut self, v: Version) -> Result<(), git2::Error> {
        ...
        let t = self
            .associated_repo
            .tag_lightweight(v.to_string().as_str(), &obj, false)?;
    ...
    fn refresh_tags(&mut self) -> () {
        ...
        let _b = &self.associated_repo.tag_foreach(|_o, n| {
            let _tag_name = match String::from_utf8(n.to_vec()) {
                ...
                Err(e) => warn!(
                    "could not convert tag to string vector dropping tag with error: {}",
                    e.to_string()
                ),
            };
        ...
        if _tags.len() > 0 {
            ...
            for t in _tags.iter() {
                match raw_tag_to_version(t) {
                    ...
                    Err(_) => {
                        warn!(
                            "tag '{}' is not semver compatible. moving to next tag...",
                            t
                        );
    ...
    pub fn bump_latest_tag(
        ...
        match i {
            Some(inc) => {
                ...
                if v.major {
                    _incremental_tag.major += inc
                }
                if v.minor {
                    _incremental_tag.minor += inc
                }
                if v.patch {
                    _incremental_tag.patch += inc
                }
            ...
            None => {
                ...
                info!("latest tag: {}", _incremental_tag);
                if v.major {
                    _incremental_tag.major += 1
                }
                if v.minor {
                    _incremental_tag.minor += 1
                }
                if v.patch {
                    _incremental_tag.patch += 1
                }
